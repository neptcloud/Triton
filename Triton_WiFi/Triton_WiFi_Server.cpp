...
/**************************************************************************/
/*
  Triton_WiFi_ClientRef implementation
*/
/**************************************************************************/

Triton_WiFi_ClientRef::Triton_WiFi_ClientRef(Triton_WiFi_Client* client)
  : _client(client) 
{ }

// Return true if the referenced client is connected.  This is provided for
// compatibility with Ethernet library code.
Triton_WiFi_ClientRef::operator bool() {
  return connected();
}
...
int Triton_WiFi_ClientRef::connect(IPAddress ip, uint16_t port) {
  HANDLE_NULL(_client, false);
  return _client->connect(ip, port);
}

int Triton_WiFi_ClientRef::connect(const char *host, uint16_t port) {
  HANDLE_NULL(_client, false);
  return _client->connect(host, port);
}

uint8_t Triton_WiFi_ClientRef::connected(void) {
  HANDLE_NULL(_client, false);
  return _client->connected();
}

size_t Triton_WiFi_ClientRef::write(uint8_t c) {
  HANDLE_NULL(_client, 0);
  return _client->write(c);
}

size_t Triton_WiFi_ClientRef::fastrprint(const char *str) {
  HANDLE_NULL(_client, 0);
  return _client->fastrprint(str);
}

size_t Triton_WiFi_ClientRef::fastrprintln(const char *str) {
  HANDLE_NULL(_client, 0);
  return _client->fastrprintln(str);
}

size_t Triton_WiFi_ClientRef::fastrprint(char *str) {
  HANDLE_NULL(_client, 0);
  return _client->fastrprint(str);
}

size_t Triton_WiFi_ClientRef::fastrprintln(char *str) {
  HANDLE_NULL(_client, 0);
  return _client->fastrprintln(str);
}

size_t Triton_WiFi_ClientRef::fastrprint(const __FlashStringHelper *ifsh) {
  HANDLE_NULL(_client, 0);
  return _client->fastrprint(ifsh);
}

size_t Triton_WiFi_ClientRef::fastrprintln(const __FlashStringHelper *ifsh) {
  HANDLE_NULL(_client, 0);
  return _client->fastrprintln(ifsh);
}

size_t Triton_WiFi_ClientRef::write(const void *buf, uint16_t len, uint32_t flags) {
  HANDLE_NULL(_client, 0);
  return _client->write(buf, len, flags);
}

int Triton_WiFi_ClientRef::read(void *buf, uint16_t len, uint32_t flags) {
  HANDLE_NULL(_client, 0);
  return _client->read(buf, len, flags);
}

int Triton_WiFi_ClientRef::read(void) {
  HANDLE_NULL(_client, 0);
  return _client->read();
}

int32_t Triton_WiFi_ClientRef::close(void) {
  HANDLE_NULL(_client, 0);
  return _client->close();
}

int Triton_WiFi_ClientRef::available(void) {
  HANDLE_NULL(_client, 0);
  return _client->available();
}

int Triton_WiFi_ClientRef::read(uint8_t *buf, size_t size) {
  HANDLE_NULL(_client, 0);
  return _client->read(buf, size);
}

size_t Triton_WiFi_ClientRef::write(const uint8_t *buf, size_t size) {
  HANDLE_NULL(_client, 0);
  return _client->write(buf, size);
}

int Triton_WiFi_ClientRef::peek() {
  HANDLE_NULL(_client, 0);
  return _client->peek();
}

void Triton_WiFi_ClientRef::flush() {
  if (_client != NULL) _client->flush();
}

void Triton_WiFi_ClientRef::stop() {
...
/**************************************************************************/
/*
  Triton_WiFi_Server implementation
*/
/**************************************************************************/

// Construct a TCP server to listen on the specified port.
Triton_WiFi_Server::Triton_WiFi_Server(uint16_t port)
  : _port(port)
  , _listenSocket(-1)
{ }
...
// for the first time.
int8_t Triton_WiFi_Server::availableIndex(bool *newClient) {
...
Triton_WiFi_ClientRef Triton_WiFi_Server::getClientRef(int8_t clientIndex) {
  if (clientIndex != -1) {
    return Triton_WiFi_ClientRef(&_clients[clientIndex]);
  }
...
size_t Triton_WiFi_Server::write(const uint8_t *buffer, size_t size) {
  size_t written = 0;
  for (int i = 0; i < MAX_SERVER_CLIENTS; ++i) {
...
// Return the sum of bytes written to all clients.
size_t Triton_WiFi_Server::write(uint8_t value) {
  return write(&value, 1);
}

// Accept new connections and update the connected clients.
bool Triton_WiFi_Server::acceptNewConnections() {
  ...
  for (int i = 0; i < MAX_SERVER_CLIENTS; ++i) {
    if (!_clients[i].connected()) {
      ...
      if (soc > -1) {
        _clients[i] = Triton_WiFi_Client(soc);
